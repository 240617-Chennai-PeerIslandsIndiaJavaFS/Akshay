...
public class ProjectTeamMembersDAO {
    ...
    // Optional: Method to get a member by name
    public TeamMembers getMemberByName(String memberName) throws SQLException {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                return new TeamMembers(memberId, teamId, name);
            } else {
                return null;
            }
        }
    }
    public List<String> getAllMembersForTeam(String teamName) throws SQLException {
        List<String> members = new ArrayList<>();
        String teamIdQuery = "SELECT team_id FROM projectteams WHERE team_name = ?";
        String membersQuery = "SELECT member_name FROM projectteammembers WHERE team_id = ?";

        try (PreparedStatement teamIdStatement = connection.prepareStatement(teamIdQuery)) {
            teamIdStatement.setString(1, teamName);
            ResultSet teamIdResultSet = teamIdStatement.executeQuery();

            if (teamIdResultSet.next()) {
                int teamId = teamIdResultSet.getInt("team_id");

                try (PreparedStatement membersStatement = connection.prepareStatement(membersQuery)) {
                    membersStatement.setInt(1, teamId);
                    ResultSet membersResultSet = membersStatement.executeQuery();

                    while (membersResultSet.next()) {
                        String memberName = membersResultSet.getString("member_name");
